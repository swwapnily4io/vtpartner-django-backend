...
@csrf_exempt
def get_service_upgrade_prices(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            ...
            query = """
                ...
                FROM vtpartner.service_plan_upgrades 
                WHERE sub_cat_id = %s AND service_id = %s 
                ORDER BY price ASC
            """
            
            result = select_query(query, [sub_cat_id, service_id])

            if not result:
                return JsonResponse({"message": "No upgrade plans found"}, status=404)
            ...
            return JsonResponse({"upgrade_prices": upgrades}, status=200)

        except Exception as e:
            print("Error fetching service upgrade prices:", e)
            return JsonResponse({"message": "Internal Server Error"}, status=500)
    ...
    if request.method == "POST":
        ...
        try:
            ...
            cab_query = """
                ...
                JOIN vtpartner.vehiclestbl ON vehiclestbl.vehicle_id = cab_bookings_tbl.cab_vehicle_id
            ...
            jcb_crane_query = """
                SELECT 
                    ...
                    sub_categorytbl.image as category_image,
            ...
            print("Goods results:", goods_results)

            print("Executing cab query with params:", [customer_id])
            cab_results = select_query(cab_query, [customer_id])
            print("Cab results:", cab_results)

            print("Executing JCB/Crane query with params:", [customer_id])
            jcb_crane_results = select_query(jcb_crane_query, [customer_id])
            print("JCB/Crane results:", jcb_crane_results)

            print("Executing driver query with params:", [customer_id])
            driver_results = select_query(driver_query, [customer_id])
            print("Driver results:", driver_results)

            print("Executing handyman query with params:", [customer_id])
            handyman_results = select_query(handyman_query, [customer_id])
            print("Handyman results:", handyman_results)
            ...
            if not all_bookings:
                return JsonResponse({
                    "status": False,
                    "message": "No scheduled bookings found"
                }, status=404)
...
def goods_driver_online_status(request):
    if request.method == "POST":
        ...
        try:
            query = """
            select is_online,status,driver_first_name,recent_online_pic,profile_pic,mobile_no,vehicle_name,image,goods_driverstbl.vehicle_id,goods_driverstbl.body_type from vtpartner.goods_driverstbl,vtpartner.vehiclestbl where vehiclestbl.vehicle_id=goods_driverstbl.vehicle_id and goods_driver_id=%s
            """
            params = [goods_driver_id]
            result = select_query(query, params)  # Assuming select_query is defined elsewhere
...
@csrf_exempt 
def get_goods_driver_new_recharge_plans_list(request):
    if request.method == "POST":
        data = json.loads(request.body)
        category_id = data.get("category_id")
        vehicle_id = data.get("vehicle_id")
...
def get_goods_driver_new_recharge_plan_history_list(request):
    if request.method == "POST":
        ...
        try:
            query = """
               select recharge_history_id,goods_driver_recharge_history_tbl.recharge_plan_id,plan_expiry_time,plan_title,plan_description,plan_days,expiry_days,plan_price,recharge_time from vtpartner.goods_driver_recharge_history_tbl,vtpartner.goods_driver_recharge_plans_tbl where goods_driver_recharge_history_tbl.driver_id=%s and goods_driver_recharge_history_tbl.recharge_plan_id=goods_driver_recharge_plans_tbl.recharge_plan_id order by recharge_history_id desc
            """
            result = select_query(query,[driver_id])  
...
@csrf_exempt 
def get_cab_driver_new_recharge_plan_history_list(request):
    if request.method == "POST":
        data = json.loads(request.body)
        driver_id = data.get("driver_id")
...
@csrf_exempt 
def new_goods_driver_new_recharge_plan(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            driver_id = data.get("driver_id")
            ...
            history_query = """
                INSERT INTO vtpartner.goods_driver_recharge_history_tbl 
            ...
            check_current_plan_query = """
                ...
                FROM vtpartner.goods_driver_current_recharge_plan_tbl 
                WHERE driver_id = %s;
            """
            ...
            try:
                ...
                if current_plan_exists:
                    ...
                    update_query = """
                        UPDATE vtpartner.goods_driver_current_recharge_plan_tbl 
                    ...
                    # Insert new plan
                    insert_query = """
                        INSERT INTO vtpartner.goods_driver_current_recharge_plan_tbl 
...
@csrf_exempt 
def jcb_crane_driver_new_recharge_plan(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            ...
            # Step 1: Insert into recharge history table
            history_query = """
                INSERT INTO vtpartner.jcb_crane_driver_recharge_history_tbl 
                (recharge_plan_id, driver_id, plan_expiry_time, razorpay_payment_id)
                VALUES (%s, %s, %s, %s)
                RETURNING recharge_history_id;
            """
            ...
            check_current_plan_query = """
                SELECT current_plan_id 
                FROM vtpartner.jcb_crane_current_recharge_plan_tbl 
                WHERE driver_id = %s;
            """
            ...
            try:
                ...
                if current_plan_exists:
                    update_query = """
                        UPDATE vtpartner.jcb_crane_current_recharge_plan_tbl 
                        SET recharge_plan_id = %s,
                            expiry_time = %s,
                            last_recharge_history_id = %s
                        WHERE driver_id = %s;
                    ...
                    insert_query = """
                        INSERT INTO vtpartner.jcb_crane_current_recharge_plan_tbl 
                        (recharge_plan_id, driver_id, expiry_time, last_recharge_history_id)
                        VALUES (%s, %s, %s, %s);
                    """
                    insert_values = [plan_id, driver_id, expiry_time, recharge_history_id]
...
@csrf_exempt 
def other_driver_new_recharge_plan(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            driver_id = data.get("driver_id")
            ...
            # Step 1: Insert into recharge history table
            history_query = """
                INSERT INTO vtpartner.other_driver_recharge_history_tbl 
                (recharge_plan_id, driver_id, plan_expiry_time, razorpay_payment_id)
                VALUES (%s, %s, %s, %s)
                RETURNING recharge_history_id;
            """
            ...
            check_current_plan_query = """
                SELECT current_plan_id 
                FROM vtpartner.other_driver_current_recharge_plan_tbl 
                WHERE driver_id = %s;
            """
            ...
            try:
                ...
                if current_plan_exists:
                    update_query = """
                        UPDATE vtpartner.other_driver_current_recharge_plan_tbl 
                        SET recharge_plan_id = %s,
                            expiry_time = %s,
                            last_recharge_history_id = %s
                        WHERE driver_id = %s;
                    ...
                    insert_query = """
                        INSERT INTO vtpartner.other_driver_current_recharge_plan_tbl 
                        (recharge_plan_id, driver_id, expiry_time, last_recharge_history_id)
                        VALUES (%s, %s, %s, %s);
                    """
                    insert_values = [plan_id, driver_id, expiry_time, recharge_history_id]
...
@csrf_exempt 
def handyman_new_recharge_plan(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            driver_id = data.get("driver_id")
            ...
            # Step 1: Insert into recharge history table
            history_query = """
                INSERT INTO vtpartner.handyman_recharge_history_tbl 
                (recharge_plan_id, driver_id, plan_expiry_time, razorpay_payment_id)
                VALUES (%s, %s, %s, %s)
                RETURNING recharge_history_id;
            """
            ...
            check_current_plan_query = """
                SELECT current_plan_id 
                FROM vtpartner.handyman_current_recharge_plan_tbl 
                WHERE driver_id = %s;
            """
            ...
            try:
                ...
                if current_plan_exists:
                    update_query = """
                        UPDATE vtpartner.handyman_current_recharge_plan_tbl 
                        SET recharge_plan_id = %s,
                            expiry_time = %s,
                            last_recharge_history_id = %s
                        WHERE driver_id = %s;
                    ...
                    insert_query = """
                        INSERT INTO vtpartner.handyman_current_recharge_plan_tbl 
                        (recharge_plan_id, driver_id, expiry_time, last_recharge_history_id)
                        VALUES (%s, %s, %s, %s);
                    """
                    insert_values = [plan_id, driver_id, expiry_time, recharge_history_id]
...
def cab_driver_online_status(request):
    if request.method == "POST":
        ...
        try:
            query = """
            select is_online,status,driver_first_name,recent_online_pic,profile_pic,mobile_no,vehicle_name,image from vtpartner.cab_driverstbl,vtpartner.vehiclestbl where vehiclestbl.vehicle_id=cab_driverstbl.vehicle_id and cab_driver_id=%s
            """
            params = [cab_driver_id]
            result = select_query(query, params)  # Assuming select_query is defined elsewhere
...
@csrf_exempt
def get_category_cancel_reasons(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            category_id = data.get("category_id")
    ...
    if request.method == "POST":
        try:
            ...
            if not booking_id or penalty_amount is None:
                return JsonResponse({
                    "success": False,
                    "message": "Booking ID and penalty amount are required"
                }, status=400)

            # Validate penalty amount is non-negative
            ...
            return JsonResponse({
                "success": False,
                "message": "Invalid penalty amount format"
            }, status=400)
        except Exception as err:
            print("Error updating booking penalty amount:", err)
            return JsonResponse({
                "success": False,
                "message": str(err)
            }, status=500)
