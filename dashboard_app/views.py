    ...
    if request.method == "POST":
        try:
            ...
            query = """
                ...
                (pincode_id, vehicle_id, price_per_km, start_time, end_time, status) 
                VALUES (%s, %s, %s, %s, %s, 1)
            """
            values = (pincode_id, vehicle_id, price_per_km, start_time, end_time)
...
def edit_peak_hour_pincodewise_price(request):
    if request.method == "POST":
        try:
            ...
            """
            values_overlap_check = (
                pincode_id, vehicle_id, peak_price_id,
                end_time, start_time,
                end_time, end_time,
                start_time, end_time
            )
...
@csrf_exempt
def get_cab_orders_report(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            start_date = data.get("start_date")
            ...
            query = """
                ...
                WHERE 
                    orders_tbl.booking_date BETWEEN %s AND %s
                ORDER BY 
                    orders_tbl.booking_date DESC, orders_tbl.booking_timing DESC
            """
            ...
            return JsonResponse({
                "results": mapped_results,
                "total_amount": total_amount,
                "total_orders": len(mapped_results)
            }, status=200)
...
@csrf_exempt
def get_other_driver_orders_report(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            start_date = data.get("start_date")
            ...
            query = """
                ...
                FROM 
                    vtpartner.other_driver_orders_tbl orders_tbl
                INNER JOIN 
                    vtpartner.other_driverstbl 
                    ON other_driverstbl.other_driver_id = orders_tbl.driver_id
                INNER JOIN 
                    vtpartner.customers_tbl 
                    ON customers_tbl.customer_id = orders_tbl.customer_id
                LEFT JOIN 
                    vtpartner.sub_categorytbl 
                    ON sub_categorytbl.sub_cat_id = orders_tbl.sub_cat_id
                LEFT JOIN 
                    vtpartner.other_servicestbl 
                    ON other_servicestbl.service_id = orders_tbl.service_id
                WHERE 
                    orders_tbl.booking_date BETWEEN %s AND %s
                ORDER BY 
                    orders_tbl.booking_date DESC, orders_tbl.booking_timing DESC
            ...
            for row in result:
                ...
                mapped_results.append({
                    ...
                    "drop_address": row[22],
                    "driver_first_name": row[23],
                    "driver_auth_token": row[24],
                    "customer_name": row[25],
                    "customer_auth_token": row[26],
                    "customer_mobile_no": row[27],
                    "driver_mobile_no": row[28],
                    "sub_cat_name": row[29],
                    "service_name": row[30]
...
@csrf_exempt
def get_jcb_crane_orders_report(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            start_date = data.get("start_date")
            ...
            query = """
                SELECT 
                    ...
                    other_servicestbl.service_name
                FROM 
                    vtpartner.jcb_crane_orders_tbl orders_tbl
                INNER JOIN 
                    vtpartner.jcb_crane_driverstbl 
                    ON jcb_crane_driverstbl.jcb_crane_driver_id = orders_tbl.driver_id
                INNER JOIN 
                    vtpartner.customers_tbl 
                    ON customers_tbl.customer_id = orders_tbl.customer_id
                LEFT JOIN 
...
@csrf_exempt
def get_handyman_orders_report(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            start_date = data.get("start_date")
            ...
            query = """
                SELECT 
                    ...
                    other_servicestbl.service_name
                FROM 
                    vtpartner.handyman_orders_tbl orders_tbl
                INNER JOIN 
                    vtpartner.handymans_tbl 
                    ON handymans_tbl.handyman_id = orders_tbl.driver_id
                INNER JOIN 
                    vtpartner.customers_tbl 
                    ON customers_tbl.customer_id = orders_tbl.customer_id
                LEFT JOIN 
            ...
            for row in result:
                ...
                mapped_results.append({
                    ...
                    "pickup_address": row[21],
                    "drop_address": row[22],
                    "handyman_name": row[23],
                    "handyman_auth_token": row[24],
                    "customer_name": row[25],
                    "customer_auth_token": row[26],
                    "customer_mobile_no": row[27],
                    "handyman_mobile_no": row[28],
                    "sub_cat_name": row[29],
                    "service_name": row[30]
                })
...
@csrf_exempt
def get_cab_scheduled_bookings_details(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            ...
            for row in result:
                mapped_results.append({
                    ...
                    "driver_first_name": str(row[26] if row[26] else "Driver Not Assigned"),
                    "cab_driver_auth_token": str(row[27]),
                    "customer_name": str(row[28]),
                    "customers_auth_token": str(row[29]),
                    "customer_mobile_no": str(row[30]),
                    "driver_mobile_no": str(row[31]) if row[31] else "Driver Not Assigned",
                    "vehicle_id": str(row[32]),
                    "vehicle_name": str(row[33]),
                    "vehicle_image": str(row[34]),
                })
...
@csrf_exempt
def get_other_driver_scheduled_bookings_details(request):
    if request.method == "POST":
        try:
            query = """
                SELECT 
                    ...
                    other_servicestbl.service_name
                FROM 
                    vtpartner.other_driver_bookings_tbl bookings_tbl
                LEFT JOIN 
                    vtpartner.other_driverstbl 
                    ON other_driverstbl.other_driver_id = bookings_tbl.driver_id
                INNER JOIN 
                    vtpartner.customers_tbl 
                    ON customers_tbl.customer_id = bookings_tbl.customer_id
                LEFT JOIN 
...
@csrf_exempt
def get_other_driver_all_ongoing_bookings_details(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            key = data.get("key")
            ...
            for row in result:
                mapped_results.append({
                    ...
                    "drop_address": str(row[24]),
                    "driver_first_name": str(row[25]),
                    "driver_auth_token": str(row[26]),
                    "customer_name": str(row[27]),
                    "customer_auth_token": str(row[28]),
                    "customer_mobile_no": str(row[29]),
                    "driver_mobile_no": str(row[30]),
...
@csrf_exempt
def get_other_driver_all_cancelled_bookings_details(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            key = data.get("key")
...
@csrf_exempt
def get_other_driver_all_completed_orders_details(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            key = data.get("key")
            ...
            for row in result:
                mapped_results.append({
                    ...
                    "drop_address": str(row[22]),
                    "driver_first_name": str(row[23]),
                    "driver_auth_token": str(row[24]),
                    "customer_name": str(row[25]),
                    "customer_auth_token": str(row[26]),
                    "customer_mobile_no": str(row[27]),
                    "driver_mobile_no": str(row[28]),
                    "sub_cat_name": str(row[29]),
                    "service_name": str(row[30]),
                })
...
@csrf_exempt
def get_other_driver_booking_detail_with_id(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            booking_id = data.get("booking_id")
            ...
            for row in result:
                mapped_results.append({
                    ...
                    "drop_address": row[24],
                    "driver_first_name": row[25],
                    "driver_auth_token": row[26],
                    "customer_name": row[27],
                    "customer_auth_token": row[28],
                    "customer_mobile_no": row[29],
                    "driver_mobile_no": row[30],
                    "sub_cat_name": row[31],
                    "service_name": row[32],
...
@csrf_exempt
def get_other_driver_order_detail_with_id(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            order_id = data.get("order_id")
...
@csrf_exempt
def get_other_driver_booking_detail_history_with_id(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            booking_id = data.get("booking_id")
            ...
            query = """
                SELECT 
                    ...
                    time,
                    date 
                FROM vtpartner.other_driver_bookings_history_tbl 
                WHERE booking_id = %s 
                ORDER BY booking_history_id ASC
            """
...
@csrf_exempt
def get_jcb_crane_scheduled_bookings_details(request):
    if request.method == "POST":
        try:
            query = """
                SELECT 
                    ...
                    other_servicestbl.service_name
                FROM 
                    vtpartner.jcb_crane_bookings_tbl bookings_tbl
                LEFT JOIN 
                    vtpartner.jcb_crane_driverstbl 
                    ON jcb_crane_driverstbl.jcb_crane_driver_id = bookings_tbl.driver_id
                INNER JOIN 
                    vtpartner.customers_tbl 
                    ON customers_tbl.customer_id = bookings_tbl.customer_id
                LEFT JOIN 
            ...
            for row in result:
                mapped_results.append({
                    ...
                    "drop_address": str(row[24]),
                    "scheduled_time": str(row[25]),
                    "driver_name": str(row[26] if row[26] else "Driver Not Assigned"),
                    "driver_auth_token": str(row[27]),
                    "customer_name": str(row[28]),
                    "customer_auth_token": str(row[29]),
                    "customer_mobile_no": str(row[30]),
...
@csrf_exempt
def get_jcb_crane_driver_all_ongoing_bookings_details(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            key = data.get("key")
...
@csrf_exempt
def get_jcb_crane_driver_all_cancelled_bookings_details(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            key = data.get("key")
...
@csrf_exempt
def get_jcb_crane_driver_all_completed_orders_details(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            key = data.get("key")
...
@csrf_exempt
def get_jcb_crane_driver_booking_detail_with_id(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            booking_id = data.get("booking_id")
...
@csrf_exempt
def get_jcb_crane_driver_order_detail_with_id(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            order_id = data.get("order_id")
...
@csrf_exempt
def get_jcb_crane_driver_booking_detail_history_with_id(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            booking_id = data.get("booking_id")
            ...
            query = """
                SELECT 
                    ...
                    time,
                    date 
                FROM vtpartner.jcb_crane_bookings_history_tbl 
                WHERE booking_id = %s 
                ORDER BY booking_history_id ASC
            """
        ...
        try:
            ...
            for row in result:
                mapped_results.append({
                    ...
                    "pickup_address": str(row[23]),
                    "drop_address": str(row[24]),
                    "handyman_name": str(row[25]),
                    "handyman_auth_token": str(row[26]),
                    "customer_name": str(row[27]),
                    "customer_auth_token": str(row[28]),
                    "customer_mobile_no": str(row[29]),
                    "handyman_mobile_no": str(row[30]),
                    "sub_cat_name": str(row[31]),
                    "service_name": str(row[32]),
                })
...
@csrf_exempt
def get_handyman_all_cancelled_bookings_details(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            key = data.get("key")
...
@csrf_exempt
def get_handyman_all_completed_orders_details(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            key = data.get("key")
            ...
            for row in result:
                mapped_results.append({
                    ...
                    "pickup_address": str(row[21]),
                    "drop_address": str(row[22]),
                    "handyman_name": str(row[23]),
                    "handyman_auth_token": str(row[24]),
                    "customer_name": str(row[25]),
                    "customer_auth_token": str(row[26]),
                    "customer_mobile_no": str(row[27]),
                    "handyman_mobile_no": str(row[28]),
                    "sub_cat_name": str(row[29]),
                    "service_name": str(row[30]),
                })
...
@csrf_exempt
def get_handyman_booking_detail_with_id(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            booking_id = data.get("booking_id")
            ...
            for row in result:
                mapped_results.append({
                    ...
                    "pickup_address": row[23],
                    "drop_address": row[24],
                    "handyman_name": row[25],
                    "handyman_auth_token": row[26],
                    "customer_name": row[27],
                    "customer_auth_token": row[28],
                    "customer_mobile_no": row[29],
                    "handyman_mobile_no": row[30],
                    "sub_cat_name": row[31],
                    "service_name": row[32],
                })
...
@csrf_exempt
def get_handyman_order_detail_with_id(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            order_id = data.get("order_id")
...
@csrf_exempt
def get_handyman_booking_detail_history_with_id(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            booking_id = data.get("booking_id")
            ...
            query = """
                SELECT 
                    ...
                    time,
                    date 
                FROM vtpartner.handyman_bookings_history_tbl 
                WHERE booking_id = %s 
                ORDER BY booking_history_id ASC
            """
...
@csrf_exempt
def handyman_current_location(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            handyman_id = data.get("handyman_id")
...
@csrf_exempt
def get_cancel_reasons(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            category_id = data.get("category_id")
            ...
            query = """
                ...
                FROM vtpartner.cancel_reasons_tbl
                WHERE category_id = %s
            ...
            print("Error fetching cancel reasons:", err)
            return JsonResponse({"message": "Internal Server Error"}, status=500)
    
    return JsonResponse({"message": "Method not allowed"}, status=405)

@csrf_exempt
def add_cancel_reason(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            required_fields = {
            ...
            query = """
                INSERT INTO vtpartner.cancel_reasons_tbl
                (reason, category_id)
                VALUES (%s, %s)
            """
            ...
            print("Error adding cancel reason:", err)
            return JsonResponse({"message": "Internal Server Error"}, status=500)
    
    return JsonResponse({"message": "Method not allowed"}, status=405)

@csrf_exempt
def update_cancel_reason(request):
    if request.method == "POST":
        try:
            data = json.loads(request.body)
            required_fields = {
            ...
            query = """
                UPDATE vtpartner.cancel_reasons_tbl
                SET reason = %s,
                    category_id = %s,
            ...
            print("Error updating cancel reason:", err)
            return JsonResponse({"message": "Internal Server Error"}, status=500)
